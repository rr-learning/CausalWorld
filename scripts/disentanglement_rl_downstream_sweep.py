...
class EncoderObservationWrapper(gym.ObservationWrapper):
    ...
    def observation(self, observation):
        goal_image_latents = self.representation_function(
            observation['goal_image'])
        real_image_latents = self.representation_function(
            observation['real_image'])
        return np.concatenate((real_image_latents, goal_image_latents),
                              axis=None)
    ...
    def __init__(self, env, camera_no=0, observation_as_dict=False):
        ...
        if self.env._robot.normalize_observations:
            self.observation_space = spaces.Box(
                low=np.zeros(shape=(height * height_multiplier, width, 3)),
...
def baseline_model(model_num):
    ...
    experiment_mode = sweep('experiment_mode',
    ...
    curriculum_kwargs = [curriculum_kwargs_1, curriculum_kwargs_2]

    return outer_product([
        experiment_mode, benchmarks, world_params, task_configs, algorithms,
        curriculum_kwargs, random_seeds, representations
    ])[model_num]
...
def get_multi_process_env(model_settings):
    ...
    def _make_env(rank):
        ...
        def _init():
            ...
            env = CameraObservationWrapper(env, camera_no=0)
            if model_settings['experiment_mode'] == 'representations':
                env = EncoderObservationWrapper(env,
                                                model_settings['module_path'])
            return env
