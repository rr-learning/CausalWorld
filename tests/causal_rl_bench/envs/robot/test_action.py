...
def test_get_action_space(as_default, as_jt_full, as_jt_norm, as_jp_full, as_jp_norm):
    ...
    assert (as_jp_full.get_action_space().low == lower_100_denormalized_jp_action).all()
    assert (as_jt_full.get_action_space().low == lower_100_denormalized_jt_action).all()
    assert (as_jp_full.get_action_space().high == upper_100_denormalized_jp_action).all()
    assert (as_jt_full.get_action_space().high == upper_100_denormalized_jt_action).all()
...
def test_clip_action(as_default, as_jt_full, as_jt_norm, as_jp_full, as_jp_norm):
    ...
    assert as_jp_full.clip_action(as_jp_full.denormalize_action(normalized_action_to_be_clipped)) == pytest.approx(
...
def test_normalize_action(as_default, as_jt_full, as_jt_norm, as_jp_full, as_jp_norm):
    ...
    assert as_jp_norm.denormalize_action(as_jp_norm.normalize_action(
        upper_100_denormalized_jp_action)) == pytest.approx(upper_100_denormalized_jp_action)
    assert as_jt_norm.denormalize_action(as_jt_norm.normalize_action(
        upper_100_denormalized_jt_action)) == pytest.approx(upper_100_denormalized_jt_action)
    assert as_jp_norm.denormalize_action(as_jp_norm.normalize_action(
        lower_100_denormalized_jp_action)) == pytest.approx(lower_100_denormalized_jp_action)
    assert as_jt_norm.denormalize_action(as_jt_norm.normalize_action(
        lower_100_denormalized_jt_action)) == pytest.approx(lower_100_denormalized_jt_action)
...
def test_denormalize_action(as_default, as_jt_full, as_jt_norm, as_jp_full, as_jp_norm):
    ...
    assert as_jp_full.denormalize_action(upper_100_normalized_action) == pytest.approx(upper_100_denormalized_jp_action)
    assert as_jp_norm.denormalize_action(upper_100_normalized_action) == pytest.approx(upper_100_denormalized_jp_action)
    assert as_jt_norm.denormalize_action(upper_100_normalized_action) == pytest.approx(upper_100_denormalized_jt_action)
    assert as_jt_full.denormalize_action(upper_100_normalized_action) == pytest.approx(upper_100_denormalized_jt_action)
    assert (as_jt_norm.denormalize_action(upper_100_normalized_action) != upper_100_denormalized_jp_action).all()
    assert (as_jt_full.denormalize_action(upper_100_normalized_action) != upper_100_denormalized_jp_action).all()
    assert (as_jp_full.denormalize_action(upper_100_normalized_action) != upper_100_denormalized_jt_action).all()
    assert (as_jp_norm.denormalize_action(upper_100_normalized_action) != upper_100_denormalized_jt_action).all()

    assert as_default.denormalize_action(lower_100_normalized_action) == pytest.approx(lower_100_denormalized_jp_action)
    assert as_jp_full.denormalize_action(lower_100_normalized_action) == pytest.approx(lower_100_denormalized_jp_action)
    assert as_jp_norm.denormalize_action(lower_100_normalized_action) == pytest.approx(lower_100_denormalized_jp_action)
    assert as_jt_full.denormalize_action(lower_100_normalized_action) == pytest.approx(lower_100_denormalized_jt_action)
    assert as_jt_norm.denormalize_action(lower_100_normalized_action) == pytest.approx(lower_100_denormalized_jt_action)
    assert (as_jt_full.denormalize_action(lower_100_normalized_action) != lower_100_denormalized_jp_action).all()
    assert (as_jt_norm.denormalize_action(lower_100_normalized_action) != lower_100_denormalized_jp_action).all()
    assert (as_jp_full.denormalize_action(lower_100_normalized_action) != lower_100_denormalized_jt_action).all()
    assert (as_jp_norm.denormalize_action(lower_100_normalized_action) != lower_100_denormalized_jt_action).all()

    # convert back and forth
    assert as_jp_norm.denormalize_action(
        as_jp_norm.normalize_action(upper_100_denormalized_jp_action)) == pytest.approx(
        upper_100_denormalized_jp_action)
    assert as_jt_norm.denormalize_action(
        as_jt_norm.normalize_action(upper_100_denormalized_jt_action)) == pytest.approx(
        upper_100_denormalized_jt_action)
    assert as_jp_norm.denormalize_action(
        as_jp_norm.normalize_action(lower_100_denormalized_jp_action)) == pytest.approx(
        lower_100_denormalized_jp_action)
    assert as_jt_norm.denormalize_action(
        as_jt_norm.normalize_action(lower_100_denormalized_jt_action)) == pytest.approx(
