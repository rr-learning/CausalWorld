...
def get_radar_data_from_experiments(experiments):
    data = []
    evaluation_protocol_labels = list(experiments[list(experiments.keys())[0]].keys())
    data.append(evaluation_protocol_labels)

    # TODO infer that from dict once more metrics are added
    metric_labels = ['mean_complete_success_rate',
                     'mean_fractional_success_rate',
                     'mean_accumulated_reward_rate']
    for metric_label in metric_labels:
        ...
        for experiment in list(experiments.keys()):
            ...
            for evaluation_protocol in evaluation_protocol_labels:
                experiment_metric_scores.append(experiments[experiment][evaluation_protocol][metric_label])
