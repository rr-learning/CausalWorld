...
class PickAndPlaceTaskGenerator(BaseTask):
    ...
    def _calculate_dense_rewards(self, desired_goal, achieved_goal):
        ...
        previous_dist_to_goal = np.linalg.norm(goal_position[:2] -
                                               self.previous_object_position[:2])
        current_dist_to_goal = np.linalg.norm(goal_position[:2] - block_position[:2])
